...
function CopyColor(){
    copy.addEventListener("click", (e) => {
        colorCode = `linear-gradient(${orientationBtn}deg, ${color1}, ${color2})`;
...
function Random(){
    ...
    randomInput.addEventListener("click", (e) => {
        e.preventDefault();
        color1 = '#'+(Math.random()*0xFFFFFF<<0).toString(16).toUpperCase();
        labelColors[0].innerHTML = color1;
        labelColors[0].style.backgroundColor = color1;
        color2 = '#'+(Math.random()*0xFFFFFF<<0).toString(16).toUpperCase();
